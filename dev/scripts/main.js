...
const brewAppGoogleKey ='AIzaSyCPJiNp9WLhIaAVBYrpsPWqCluZWalWjj8';
const brewAppGoogleApi ='https://maps.googleapis.com/maps/api/';
var city;
var breweryLocation = [];
var userResults; 
var shuffleResults;
...
// Get results
	...
	brewApp.separateVal = function(data) {
		...
		if (data.indexOf(',') !== -1) {// if there is a comma, then split it by a comma 
...
brewApp.googleMarkers = function(markers){
	function setMarkers(map){
		markers.forEach(function(markersBrew){// for each array go through it and feed a result to the map 
